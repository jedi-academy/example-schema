...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:key name="instant" match="instance" use="../../@airline"/>
    ...
    <xsl:template name="analysis">
        ...
        <table>
            ...
            <xsl:for-each select="/schedule/flight/instance">
            ...
            <tr>
                ...
                <xsl:variable name="total-distance" select="sum(//instance/../@distance)"/>
                ...
                <xsl:variable name="number-flights" select="count(//instance)"/>
    ...
    <xsl:template name="flight">
        ...
        <xsl:variable name="departing-name">
            <xsl:apply-templates select="$airports-top">
                <xsl:with-param name="airport-code" select="../@from"/>
            </xsl:apply-templates>
        </xsl:variable>
        <!-- lookup the destination airport -->
        <xsl:variable name="arriving-name">
            <xsl:apply-templates select="$airports-top">
                <xsl:with-param name="airport-code" select="../@to"/>
            </xsl:apply-templates>
        </xsl:variable>
    ...
    <!-- Use an XSL table lookup, with a different namespace, to get the airport name -->
    <xsl:key name="airport-lookup" match="ac:airport" use="ac:code"/>
    <xsl:variable name="airports-top" select="document('')/*/ac:airports"/>
    <xsl:template match="ac:airports">
        <xsl:param name="airport-code"/>
        <xsl:value-of select="key('airport-lookup', $airport-code)/ac:name"/>
    </xsl:template>
    <ac:airports>
        <ac:airport>
            <ac:code>YVR</ac:code>
            <ac:name>Vancouver</ac:name>
        </ac:airport>
        <ac:airport>
            <ac:code>YYC</ac:code>
            <ac:name>Calgary</ac:name>
        </ac:airport>
        <ac:airport>
            <ac:code>TTZ</ac:code>
            <ac:name>Toronto</ac:name>
        </ac:airport>
    </ac:airports>
